...
/** 진행 단계 스텝퍼 (28-8-01): 낙찰 → 방문 예정 → 시공 중 → 작업 완료 → 정산 완료 */
const WORK_STEPS: JobStatus[] = ['accepted', 'scheduled', 'in_progress', 'completed', 'settled']

function WorkStepper({ status }: { status: JobStatus }) {
  const current = WORK_STEPS.indexOf(status)
  if (current < 0) return null
  return (
    <div className="stepper">
      {WORK_STEPS.map((step, i) => (
        <div key={step} className={`step ${i < current ? 'done' : ''} ${i === current ? 'now' : ''}`}>
...
function Dday({ deadline, today }: { deadline: string | undefined; today: string }) {
  if (!deadline) return null
  const diff = Math.round(
    (Date.parse(`${deadline}T00:00:00Z`) - Date.parse(`${today}T00:00:00Z`)) / 86400000,
  )
  const label = diff < 0 ? '마감' : diff === 0 ? 'D-Day' : `D-${diff}`
  const cls = diff < 0 ? 'dday closed' : diff <= 2 ? 'dday urgent' : 'dday'
...
type Tab = 'dashboard' | 'jobs' | 'work' | 'schedule' | 'settlement' | 'my'
  ...
  return (
    <div className="app">
      <header className="app-header">
        <span className="logo-mark">반</span>
        <h1>반듯집수리 파트너</h1>
        <span className="round">28-9</span>
      </header>
      <nav className="tabs">
      ...
      <main className="panel">
        ...
        {tab === 'my' && <MyProfile platform={platform} onChange={refresh} />}
      </main>
    </div>
  )
}
...
function Jobs({ platform, onChange }: { platform: PlatformService; onChange: () => void }) {
  ...
  const [error, setError] = useState('')
  // 28-7-03: 카드의 "이 일감에 입찰" → 폼 대상 선택과 연동
  const [bidTarget, setBidTarget] = useState('')

  const pickForBid = (id: string) => {
    setBidTarget(id)
  ...
  return (
    <div>
      ...
      <BidForm
        platform={platform}
        onChange={onChange}
        selectedId={bidTarget}
        onSelect={setBidTarget}
      />
        ...
        return (
          <div key={r.id} className="card">
            <div className="row">
              <div>
                ...
                <div className="meta">
                  ...
                  {r.budget !== undefined && <> · 예산 <strong>{krw(r.budget)}</strong></>}
                  {' '}<Dday deadline={r.bidDeadline} today={today} />
                </div>
                {bid && (
                  <div className="meta">
            ...
            <div className="row" style={{ marginTop: 10, justifyContent: 'flex-start' }}>
              {r.status === 'requested' && (
                <button onClick={() => pickForBid(r.id)}>이 일감에 입찰</button>
              )}
              {r.status === 'estimated' && (
                <>
                  <button className="btn-secondary" onClick={() => pickForBid(r.id)}>
                    제안 수정
                  </button>
                  <span className="meta">고객 결정 시뮬레이션:</span>
                  <button onClick={() => decide(r, 'accepted')}>낙찰</button>
                  <button className="btn-danger" onClick={() => decide(r, 'lost')}>
                    미낙찰
                  </button>
                </>
              )}
...
function BidForm({
  ...
  return (
    <div className="card form" style={{ marginBottom: 12 }}>
      ...
      <label>
        일감 선택
        <select value={requestId} onChange={(e) => onSelect(e.target.value)}>
      ...
      {active.map((r) => {
        ...
        return (
          <div key={r.id} className="card">
            ...
            <WorkStepper status={r.status} />
            {['accepted', 'scheduled', 'in_progress'].includes(r.status) && (
              <div className="checklist">
                {platform.checklistFor(r.id).map((item) => (
                  <label key={item.key} className={item.done ? 'done' : ''}>
                    <input
                      type="checkbox"
                      checked={item.done}
                      onChange={() => {
                        platform.toggleChecklistItem(r.id, item.key)
                        onChange()
                      }}
                    />
                    {item.label}
...
function Dashboard({ platform, onChange }: { platform: PlatformService; onChange: () => void }) {
  ...
  const inProgressCount = counts.scheduled + counts.in_progress
  return (
    <div>
      <div className="stat-grid">
        <div className="stat">
          <div className="label">오늘의 작업</div>
          <div className="value">{todaySlots.length}<small>건</small></div>
        </div>
        <div className="stat">
          <div className="label">입찰 가능</div>
          <div className="value">{counts.requested}<small>건</small></div>
        </div>
        <div className="stat">
          <div className="label">진행 중</div>
          <div className="value">{inProgressCount}<small>건</small></div>
        </div>
        <div className="stat accent">
          <div className="label">정산 예정 금액</div>
          <div className="value">{krw(unsettled)}</div>
        </div>
      </div>
      <div className="card">
        ...
        {todaySlots.map((s) => {
          ...
          return (
            <div key={s.id} className="meta" style={{ padding: '4px 0' }}>
              ...
              {req && <span style={{ marginLeft: 6 }}><StatusChip status={req.status} /></span>}
            </div>
          )
        })}
      </div>
...
/** 마이 탭 (28-8-04): 파트너 프로필 등록/수정 */
function MyProfile({ platform, onChange }: { platform: PlatformService; onChange: () => void }) {
  const saved = platform.getProfile()
  ...
  const submit = () => {
    setMessage('')
    try {
      const profile: PartnerProfile = { name, specialties, region, intro }
      platform.updateProfile(profile)
  ...
  return (
    <div>
        ...
        <div className="card">
          <div className="row">
            <div>
              <h3>{saved.name}</h3>
              <div className="meta">
                {saved.specialties && <>전문 분야: {saved.specialties} · </>}
                {saved.region && <>서비스 지역: {saved.region}</>}
              </div>
              {saved.intro && <div className="meta">{saved.intro}</div>}
            </div>
            <span className="chip st-accepted">파트너 {PARTNER_ID}</span>
          </div>
        </div>
      )}
      <div className="card form">
        <h3 style={{ margin: 0 }}>{saved ? '프로필 수정' : '프로필 등록'}</h3>
        <label>
          업체명 (필수)
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="예: 반듯인테리어" />
        </label>
        <label>
          전문 분야
          <input value={specialties} onChange={(e) => setSpecialties(e.target.value)} placeholder="예: 도배, 타일, 욕실" />
        </label>
        <label>
          서비스 지역
          <input value={region} onChange={(e) => setRegion(e.target.value)} placeholder="예: 서울 전체, 경기 일부" />
        </label>
        <label className="form-wide">
          소개
          <input value={intro} onChange={(e) => setIntro(e.target.value)} placeholder="예: 꼼꼼한 시공, 정직한 견적을 약속드립니다" />
        </label>
      ...
      <div className="card">
        <h3>체크리스트 항목 안내</h3>
        <div className="meta">
          낙찰된 작업 카드에는 다음 체크리스트가 제공됩니다: {CHECKLIST_ITEMS.map((i) => i.label).join(' · ')}
        </div>
      </div>
    </div>
  )
