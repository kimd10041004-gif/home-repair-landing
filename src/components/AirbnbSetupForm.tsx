...
import {
  AIRBNB_SETUP,
  AIRBNB_SETUP_CONSENTS,
  AIRBNB_SETUP_PACKAGES,
  AIRBNB_SETUP_WORK_ITEMS,
  TENANT_CARE_RESIDENCE_TYPES,
} from "@/lib/constants";

/**
 * 에어비앤비 세팅 케어 전용 상담 신청 폼.
 ...
 */
export default function AirbnbSetupForm() {
  ...
  const [packageId, setPackageId] = useState(AIRBNB_SETUP_PACKAGES[0].id);
  ...
  const requiredConsents = AIRBNB_SETUP_CONSENTS.filter((c) => c.required);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/airbnb-setup", {
        method: "POST",
  ...
  if (done) {
    return (
      <div className="rounded-lg border border-brand-teal/30 bg-brand-cream p-6 text-center">
        <p className="font-semibold text-brand-navy">{AIRBNB_SETUP.submittedNotice1}</p>
        <p className="mt-2 text-sm text-slate-600">{AIRBNB_SETUP.submittedNotice2}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="font-medium text-slate-700">건물 소유 형태</span>
          <select
            className="rounded-md border border-slate-300 px-3 py-2"
            value={residenceType}
      ...
      <fieldset className="rounded-md border border-slate-200 p-3">
        ...
        <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
          {AIRBNB_SETUP_WORK_ITEMS.map((item) => (
      ...
      <div className="rounded-md border border-slate-200 p-4">
        ...
        <div className="mt-2 flex flex-col gap-2">
          {AIRBNB_SETUP_CONSENTS.filter((c) => c.required).map((c) => (
