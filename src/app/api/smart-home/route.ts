 ...
 * 이 프로젝트의 DB에는 저장하지 않고, 기존 /api/estimate·/api/tenant-care와 동일하게
 * home-repair-promo의 write-only 전용 엔드포인트로 그대로 전달(proxy)만 한다.
 *
 * ⚠️ 참고: 스마트홈 IoT 케어는 이번에 신설된 서비스로, 전용 접수 엔드포인트가
 * backend(home-repair-promo)에 아직 준비되지 않았을 가능성이 매우 높다.
 * 준비 여부는 배포 전 별도 확인이 필요하며, 미준비 시 이 라우트는 404/오류를
 * 반환할 수 있다 (최종 보고서에 명시).
 */
const SMART_HOME_ESTIMATE_ENDPOINT =
  process.env.SMART_HOME_ESTIMATE_ENDPOINT ??
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const missingRequired = requiredConsentIds.some((id) => !consentMap[id]);
    if (missingRequired) {
      return NextResponse.json(
    ...
    const upstreamRes = await fetch(SMART_HOME_ESTIMATE_ENDPOINT, {
      method: "POST",
    ...
    console.error("[api/smart-home] proxy failed", error);
    return NextResponse.json(
